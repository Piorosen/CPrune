...
import json
import copy
from utils import get_data_dataset
import pickle
...
def evaluate_tvm(tasks, mod, params, dummy_input, input_size, input_name, device_type, tvm_target, tvm_device_key, tvm_host, tvm_port, log_file):
    print("Compile...")
    ...
    data_tvm = tvm.nd.array((dummy_input.numpy()).astype('float32'))
    ...
    return prof_res, module.get_output(0).asnumpy().tolist()
...
def main():
    id = 1
    _, val_loader, criterion = get_data_dataset('imagenet', '/work/dataset', 1, 10)
    dummy_input = enumerate(val_loader).__next__()
    dummy_input = dummy_input[1][0]
    with open('dummy_data.pkl', 'wb') as f:
        pickle.dump(dummy_input, f)
        
    for id in range(1, 100):
        dirr = '/work/experiments/fast_resnet18/tvm'
        path = _get_last_epoch(id, dirr)
        dirs = os.path.join(dirr, path)
        ...
        args.model_pth = dirs + "_model.pth"
        args.mask_pth = dirs + "_mask.pth"
        args.tvm_log = dirs + ".log"
        ...
        # _, val_loader, criterion = get_data_dataset('imagenet', '/work/dataset', 1, 100)
        # dummy_input = enumerate(val_loader).__next__()
        # dummy_input = dummy_input[1][0]
        ...
        dur, result = evaluate_tvm(tasks, mod, params, dummy_input, input_shape, input_name, 'cpu', args.tvm_target, args.tvm_devicekey, args.tvm_host, args.tvm_port, args.tvm_log)
        # with open('100_000000.ll','wt') as f:
        #     f.write(llvm_ir)
        # exit(0)
        
        with open(f'data_{path}.pkl', 'wb') as f:
            pickle.dump([dur, result, infer_cpu], f)
            # json.dump({'inference': llvm_ir, 'info': info, 'param': vars(args)}, f)
    # print(rrr)
#%%
...
result = []
# with open('data_001_000000.pkl', 'rb') as f:
    # a0 = np.array(pickle.load(f)[1]).flatten()
dirr = '/work/experiments/fast_resnet18/tvm'
for i in range(1, 59):
    path = _get_last_epoch(i, dirr)
    with open(f'data_{path}.pkl', 'rb') as f:
        d = pickle.load(f)
        a0 = np.array(d[1]).flatten()
        a1 = d[2].detach().numpy().flatten()
    score = np.mean((a0 - a1) ** 2)
    result.append(score)
print(np.max(result))
...
with open('a_000_000000.pkl', 'rb') as f:
    a0 = pickle.load(f)
    # a0 = '\n'.join(f.readlines())
with open('a_009_000002.pkl', 'rb') as f:
    a1 = pickle.load(f)
    # a1 = '\n'.join(f.readlines())
...
#%%
diff = difflib.Differ()
dif = diff.compare(str(a0).splitlines(), str(a1).splitlines())
print("\n".join(dif))
# %%
a0[id]
# %%
